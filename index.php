...
define('SPH_KEY','KunciRahasiaSPH-MAN2PLG-2026');
function encPass($p){$k=substr(hash('sha256',SPH_KEY),0,32);$iv=substr(hash('sha256','SPH-IV'),0,16);return base64_encode(openssl_encrypt($p,'aes-256-cbc',$k,0,$iv));}
...
function sigBlock($ex,$set){return '<div class="sig" style="width:90%;margin:40px auto 0;padding:0 12px"><div>Wali Kelas,<br><br><br><br><b><u>'.e($ex['wali']?:'........................').'</u></b><br>NIP. '.e($ex['nip_wali']?:'-').'</div><div>Palembang, '.tglIndo($ex['tanggal']).'<br>Guru Mata Pelajaran,<br><br><br><br><b><u>'.e($ex['guru']?:'........................').'</u></b><br>NIP. '.e($ex['nip_guru']?:'-').'</div><div>Mengesahkan, Kepala Madrasah,<br><br><br><br><b><u>'.e($set['kepala']?:'........................').'</u></b><br>NIP. '.e($set['nip_kepala']?:'-').'</div></div>';}
function sigResmi($ex,$set){return '<table class="info" style="margin-top:26px;width:92%;margin-left:auto;margin-right:auto"><tr><td style="width:50%;vertical-align:top">Mengetahui<br>Waka Bidang Kurikulum dan Evaluasi,<br><br><br><br><b><u>'.e($set['waka']??'............................').'</u></b><br>NIP. '.e($set['nip_waka']??'-').'</td><td style="width:50%;vertical-align:top">Palembang, '.tglIndo($ex['tanggal']).'<br>Guru Mata Pelajaran,<br><br><br><br><b><u>'.e($ex['guru']?:'............................').'</u></b><br>NIP. '.e($ex['nip_guru']?:'-').'</td></tr><tr><td colspan="2" style="padding-top:12px">Mengesahkan<br>Kepala Madrasah,<br><br><br><br><b><u>'.e($set['kepala']?:'............................').'</u></b><br>NIP. '.e($set['nip_kepala']?:'-').'</td></tr></table>';}
function sigAnalisis($ex,$set){return '<table class="info" style="margin-top:26px;width:94%;margin-left:auto;margin-right:auto"><tr><td style="width:33%;vertical-align:top">Mengetahui<br>Kepala Madrasah,<br><br><br><br><b><u>'.e($set['kepala']?:'............................').'</u></b><br>NIP. '.e($set['nip_kepala']?:'-').'</td><td style="width:34%;vertical-align:top">Waka Bidang Kurikulum dan Evaluasi,<br><br><br><br><b><u>'.e($set['waka']??'............................').'</u></b><br>NIP. '.e($set['nip_waka']??'-').'</td><td style="width:33%;vertical-align:top">Palembang, '.tglIndo($ex['tanggal']).'<br>Guru Mata Pelajaran,<br><br><br><br><b><u>'.e($ex['guru']?:'............................').'</u></b><br>NIP. '.e($ex['nip_guru']?:'-').'</td></tr></table>';}
...
function httpPost_($u,$d){$ch=curl_init($u);curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>1,CURLOPT_POST=>1,CURLOPT_POSTFIELDS=>json_encode($d),CURLOPT_HTTPHEADER=>['Content-Type: text/plain;charset=utf-8'],CURLOPT_FOLLOWLOCATION=>1,CURLOPT_TIMEOUT=>90]);$r=curl_exec($ch);curl_close($ch);return json_decode($r?:'null',true);}
function httpGet_($u){$ch=curl_init($u);curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>1,CURLOPT_FOLLOWLOCATION=>1,CURLOPT_TIMEOUT=>90]);$r=curl_exec($ch);curl_close($ch);return json_decode($r?:'null',true);}
...
if($_SERVER['REQUEST_METHOD']==='POST'){$act=$_POST['action']??'';
 ...
 elseif($logged){
  if($act=='change_pass'){$u=$conn->query("SELECT * FROM users WHERE id=$uid")->fetch_assoc();if($u&&password_verify($_POST['old']??'',$u['pass_hash'])){$h=password_hash($_POST['new'],PASSWORD_DEFAULT);$conn->query("UPDATE users SET pass_hash='".$conn->real_escape_string($h)."',pass_plain='".$conn->real_escape_string(encPass($_POST['new']))."' WHERE id=$uid");$msg='Password Anda diganti.';}else{$msg='Password lama salah!';}}
  if($act=='change_pass_admin'&&$isAdmin){
   $u=$conn->query("SELECT * FROM users WHERE id=$uid")->fetch_assoc();
   $old=$_POST['old']??'';$new1=$_POST['new1']??'';$new2=$_POST['new2']??'';
   if(!password_verify($old,$u['pass_hash'])){$msg='Password lama salah!';}
   elseif(strlen($new1)<6){$msg='Password baru minimal 6 karakter!';}
   elseif($new1!==$new2){$msg='Konfirmasi password tidak cocok!';}
   else{$h=password_hash($new1,PASSWORD_DEFAULT);$conn->query("UPDATE users SET pass_hash='".$conn->real_escape_string($h)."',pass_plain='".$conn->real_escape_string(encPass($new1))."' WHERE id=$uid");$msg='Password admin berhasil diganti. Silakan login ulang bila perlu.';}}
  ...
  elseif($act=='save_manual'){$eid=(int)$_POST['exam_id'];$ex=$conn->query("SELECT * FROM exams WHERE id=$eid")->fetch_assoc();
    ...
    elseif($act=='buat_template'){$eid=(int)$_POST['exam_id'];$ex=$conn->query("SELECT * FROM exams WHERE id=$eid")->fetch_assoc();
if($ex&&($isAdmin||(int)$ex['dibuat_oleh']===$uid)){
if(empty($set['gas_url'])||empty($set['gas_secret'])){$msg='Isi dulu GAS URL & Secret di Pengaturan admin.';}
else{$kelas=$_POST['kelas_peserta']??[];$peserta=[];foreach($kelas as $kk){$q=$conn->query("SELECT nisn,nama,kelas,tgl_lahir FROM students WHERE kelas='".$conn->real_escape_string($kk)."' ORDER BY nama");while($s=$q->fetch_assoc())$peserta[]=$s;}
$code=$ex['exam_code']?:strtoupper(substr(md5($eid.time()),0,8));
$pin=$ex['pin_guru']?:str_pad(random_int(0,9999),4,'0',STR_PAD_LEFT);
$token=$ex['token']?:strtoupper(substr(md5(uniqid()),0,6));
$r=httpPost_($set['gas_url'],['secret'=>$set['gas_secret'],'action'=>'create_exam','exam_id'=>$eid,'code'=>$code,'mapel'=>$ex['mapel'],'kelas'=>$ex['kelas'],'tanggal'=>$ex['tanggal'],'kkm'=>$ex['kkm'],'skor_per_soal'=>$ex['skor_per_soal'],'nama_ujian'=>'PH '.strtoupper($ex['mapel']).' '.$ex['kelas'],'pin'=>$pin,'token'=>$token,'durasi'=>60,'kelas_list'=>$kelas,'peserta'=>$peserta]);
if($r&&$r['ok']){$conn->query("UPDATE exams SET kelas_peserta='".$conn->real_escape_string(implode(',',$kelas))."',exam_code='".$conn->real_escape_string($code)."',pin_guru='".$conn->real_escape_string($pin)."',token='".$conn->real_escape_string($token)."',link_guru='".$conn->real_escape_string($r['link_guru'])."',link_siswa='".$conn->real_escape_string($r['link_siswa'])."',link_sheet='".$conn->real_escape_string($r['sheet_url'])."' WHERE id=$eid");$msg='✔ Template dibuat di Drive admin. Link & PIN tampil di tabel.';}
else{$msg='Gagal: '.e($r['err']??'tidak ada respons GAS. Cek URL/Secret/deployment.');}}}}
elseif($act=='tarik_hasil'){$eid=(int)$_POST['exam_id'];$ex=$conn->query("SELECT * FROM exams WHERE id=$eid")->fetch_assoc();
if($ex&&($isAdmin||(int)$ex['dibuat_oleh']===$uid)&&$ex['exam_code']){
$r=httpGet_($set['gas_url'].'?action=results&code='.urlencode($ex['exam_code']).'&key='.urlencode($set['gas_secret']));
if($r&&$r['ok']){$n=$ex['jumlah_soal'];$kunci=$r['kunci'];$conn->query("DELETE FROM results WHERE exam_id=$eid");$cnt=0;
foreach($r['hasil'] as $h){$nisn=$conn->real_escape_string($h['nisn']);$s2=$conn->query("SELECT * FROM students WHERE nisn='$nisn'")->fetch_assoc();$nama=$s2?$s2['nama']:$h['nama'];$kelas=$s2?$s2['kelas']:$h['kelas'];
$ans=str_split(strtoupper($h['jawaban']));$b=0;$det='';for($i=0;$i<$n;$i++){$okk=isset($ans[$i])&&isset($kunci[$i])&&$ans[$i]===$kunci[$i];$det.=$okk?'1':'0';if($okk)$b++;}
$skor=(int)round($b*(float)$ex['skor_per_soal']);$status=$skor>=$ex['kkm']?'TUNTAS':'REMEDIAL';
$conn->query("INSERT INTO results (exam_id,nisn,nama,kelas,jawaban,detail,benar,skor,status) VALUES ($eid,'$nisn','".$conn->real_escape_string($nama)."','".$conn->real_escape_string($kelas)."','".$conn->real_escape_string(implode('',$ans))."','$det',$b,$skor,'$status')");$cnt++;}
$conn->query("UPDATE exams SET mode='CBT' WHERE id=$eid");$msg=$cnt.' hasil ditarik dari Google & terkoreksi otomatis.';}
else{$msg='Gagal menarik hasil: '.e($r['err']??'periksa GAS URL/Secret/kode.');}}
else{$msg='Ujian belum punya template. Klik 🎓 dulu.';}}
  /* ===== KHUSUS ADMIN ===== */
  if($isAdmin){
   if($act=='save_settings'){$conn->query("UPDATE settings SET sekolah='".$conn->real_escape_string($_POST['sekolah'])."',ta='".$conn->real_escape_string($_POST['ta'])."',smt='".$conn->real_escape_string($_POST['smt'])."',kkm=".(int)$_POST['kkm'].",kepala='".$conn->real_escape_string($_POST['kepala'])."',nip_kepala='".$conn->real_escape_string($_POST['nip_kepala'])."',waka='".$conn->real_escape_string($_POST['waka']??'')."',nip_waka='".$conn->real_escape_string($_POST['nip_waka']??'')."',gas_url='".$conn->real_escape_string($_POST['gas_url']??'')."',gas_secret='".$conn->real_escape_string($_POST['gas_secret']??'')."'");$set=$conn->query("SELECT * FROM settings LIMIT 1")->fetch_assoc();$msg='Pengaturan tersimpan.';}
   elseif($act=='add_siswa'){$n=$conn->real_escape_string(trim($_POST['nisn']));$m=$conn->real_escape_string(trim($_POST['nama']));$k=$conn->real_escape_string(normKelas($_POST['kelas']));$t=$conn->real_escape_string(trim($_POST['tgl_lahir']??''));$conn->query("INSERT INTO students (nisn,nama,kelas,tgl_lahir) VALUES ('$n','$m','$k','$t') ON DUPLICATE KEY UPDATE nama='$m',kelas='$k',tgl_lahir='$t'");$msg='Siswa tersimpan.';}
   ...
   elseif($act=='del_siswa'){$conn->query("DELETE FROM students WHERE nisn='".$conn->real_escape_string($_POST['nisn'])."'");}
     ...
     foreach($rows as $r){list($nip,$nama,$mapel,$user,$pass)=$r;if(!$nip||!$nama)continue;if($pass==='')$pass='guru123';if($user==='')$user=makeUsername($conn,$nama,$nip);$h=password_hash($pass,PASSWORD_DEFAULT);
       $conn->query("INSERT INTO users (username,pass_hash,pass_plain,nama,nip,mapel,role) VALUES ('".$conn->real_escape_string($user)."','".$conn->real_escape_string($h)."','".$conn->real_escape_string(encPass($pass))."','".$conn->real_escape_string($nama)."','".$conn->real_escape_string($nip)."','".$conn->real_escape_string($mapel)."','guru') ON DUPLICATE KEY UPDATE pass_hash=VALUES(pass_hash),pass_plain=VALUES(pass_plain),nama=VALUES(nama),mapel=VALUES(mapel)");$cnt++;}
   ...
   elseif($act=='del_guru'){$conn->query("DELETE FROM users WHERE id=".(int)$_POST['uid']." AND role='guru'");$msg='Akun guru dihapus.';}
      elseif($act=='reset_pass'){$np=trim((string)($_POST['newpass']??''));if($np===''){$np=substr(str_shuffle('abcdefghjkmnpqrstuvwxyz23456789'),0,6);}$h=password_hash($np,PASSWORD_DEFAULT);$conn->query("UPDATE users SET pass_hash='".$conn->real_escape_string($h)."',pass_plain='".$conn->real_escape_string(encPass($np))."' WHERE id=".(int)$_POST['uid']." AND role='guru'");$msg='Password guru direset. Password baru: <b>'.$np.'</b> (tersimpan terenkripsi, dapat dilihat lewat tombol 👁).';}
...
/* ================= TEMPLATE & BACKUP & CSV ================= */
if($logged&&$isAdmin&&isset($_GET['tpl'])){$t=$_GET['tpl'];header('Content-Type:text/csv');header('Content-Disposition:attachment;filename="template_'.$t.'.csv"');echo "\xEF\xBB\xBF";if($t==='siswa')echo "NISN;NAMA;KELAS;TGL_LAHIR\n0089526958;zahara pebriyanti;XII.3;2006-02-15\n";else echo "NIP;NAMA;MAPEL;USERNAME;PASSWORD\n198001012005011001;MARLAYLI S.Pd;Fiqih;;guru123\n";exit;}
...
if($logged&&!$isAdmin&&in_array($p,['guru','kelas','kkm'])){$p='home';$msg='Halaman khusus Admin.';}
...
<div class="hint">Default admin: <b>MAN 2 PALEMBANG</b></div></form></body></html>
<?php exit;} ?>
...
table.tbl{border-collapse:separate;border-spacing:0;border:1px solid #cfd8e3;border-radius:10px;overflow:hidden}
table.tbl thead th{background:#1a3e72;color:#fff;padding:9px 8px;border:none}
table.tbl tbody td{padding:8px;border-bottom:1px solid #e3e9f2}
table.tbl tbody tr:nth-child(even){background:#f4f8ff}
table.tbl tbody tr:hover{background:#e8f1e4}
table.tbl code{background:#eef2f7;padding:2px 6px;border-radius:4px;font-size:11px}
.badge{background:#2e7d32;color:#fff;border-radius:12px;padding:3px 10px;font-size:11px;font-weight:bold}
...
<nav class="no-print">
<?php
$NAV=[['home','🏠 BERANDA'],['master',$isAdmin?'🗂 MASTER SISWA':'🗂 DATA SISWA']];
if($isAdmin){$NAV[]=['guru','👥 DATA GURU & AKUN'];$NAV[]=['kelas','🏫 KELAS & WALI'];$NAV[]=['kkm','🎯 RENTANG PREDIKAT'];}
...
<?php if($isAdmin){?><div class="card"><h2>Pengaturan Madrasah (Admin)</h2><form method="post"><input type="hidden" name="action" value="save_settings">
 ...
 <div class="row"><div><label>Kepala Madrasah</label><input name="kepala" value="<?=e($set['kepala'])?>"></div><div><label>NIP Kepala</label><input name="nip_kepala" value="<?=e($set['nip_kepala'])?>"></div></div>
   ...
   <div class="row"><div><label>Google Apps Script URL (Web App)</label><input name="gas_url" value="<?=e($set['gas_url']??'')?>" placeholder="https://script.google.com/macros/s/…/exec"></div><div><label>GAS Secret</label><input name="gas_secret" value="<?=e($set['gas_secret']??'')?>" placeholder="sama dengan SECRET di Code.gs"></div></div>
  <div class="row"><div><label>Google Apps Script URL (Web App)</label><input name="gas_url" value="<?=e($set['gas_url']??'')?>"></div><div><label>GAS Secret</label><input name="gas_secret" value="<?=e($set['gas_secret']??'')?>"></div></div>
 ...
 document.getElementById('wakaSel').onchange=updWaka;updWaka();
 </script>
 </div><?php } } ?>
...
<div class="card"><h2>Tambah Manual</h2><form method="post"><input type="hidden" name="action" value="add_siswa"><div class="row"><div><label>NISN</label><input name="nisn" required></div><div><label>Nama</label><input name="nama" required></div><div><label>Kelas</label><input name="kelas" placeholder="XII.3"></div><div><label>Tgl Lahir (YYYY-MM-DD)</label><input type="date" name="tgl_lahir"></div><div style="flex:0"><label>&nbsp;</label><button class="btn">+ Tambah</button></div></div></form>
</div>
<?php } else { ?><div class="note" style="margin-bottom:10px">👁 <b>Mode baca saja</b> — data siswa dikelola oleh Admin. Hubungi admin untuk penambahan/perubahan data.</div><?php } ?>
<div class="card"><h2>📋 Data Siswa <?=$isAdmin?'':'(Mode Baca)'?></h2>
<?php
 $cari=trim($_GET['cari']??'');$fkelas=trim($_GET['kelas']??'');
 $where=[];if($cari!=='')$where[]="CONCAT(nisn,nama,kelas) LIKE '%".$conn->real_escape_string($cari)."%'";if($fkelas!=='')$where[]="kelas='".$conn->real_escape_string($fkelas)."'";
 $wsql=$where?'WHERE '.implode(' AND ',$where):'';
 $totAll=$conn->query("SELECT COUNT(*) c FROM students")->fetch_assoc()['c'];
...
<?php if($isAdmin){ ?><script>document.getElementById('fileSiswa').onchange=function(ev){var f=ev.target.files[0];if(!f)return;var r=new FileReader();r.onload=function(e2){var wb=XLSX.read(e2.target.result,{type:'array'});var ws=wb.Sheets[wb.SheetNames[0]];document.getElementById('paste_siswa').value=XLSX.utils.sheet_to_csv(ws,{FS:'\t'});};r.readAsArrayBuffer(f);};</script><?php } ?>
<?php } ?>
...
<form method="post"><input type="hidden" name="action" value="save_exam"><input type="hidden" name="edit_id" value="<?=$editEx['id']??0?>">
 ...
 <div class="note">Isikan minimal 1 Tema; gunakan <b>huruf kecil</b>; Tema melekat pada mapel + kelas + tanggal ujian ini.</div>
...
<?php $pesId=(int)($_GET['peserta']??0);if($pesId){$exP=$conn->query("SELECT * FROM exams WHERE id=$pesId")->fetch_assoc();if($exP){$curK=explode(',',$exP['kelas_peserta']??'');$allK=[];$q=$conn->query("SELECT DISTINCT kelas FROM students WHERE kelas<>''");while($x=$q->fetch_assoc())$allK[]=$x['kelas'];$q=$conn->query("SELECT nama_kelas FROM kelas");while($x=$q->fetch_assoc())if(!in_array($x['nama_kelas'],$allK))$allK[]=$x['nama_kelas'];usort($allK,function($a,$b){return kelasRank($a)-kelasRank($b);});?>
<div class="card"><h2>🎓 Kelas Peserta — <?=e($exP['mapel'])?> <?=e($exP['kelas'])?></h2>
<form method="post"><input type="hidden" name="action" value="buat_template"><input type="hidden" name="exam_id" value="<?=$pesId?>">
<div style="display:flex;flex-wrap:wrap;gap:10px"><?php foreach($allK as $kk)echo '<label style="font-weight:normal;margin:0"><input type="checkbox" name="kelas_peserta[]" value="'.e($kk).'" '.(in_array($kk,$curK)?'checked':'').'> '.e($kk).'</label>';?></div>
<button class="btn green">📤 Kirim & Buat Template di Google Drive</button> <a class="btn gray" href="?p=setup">Batal</a></form></div><?php }} ?>
<?php $pesId=(int)($_GET['peserta']??0);if($pesId){$exP=$conn->query("SELECT * FROM exams WHERE id=$pesId")->fetch_assoc();if($exP&&($isAdmin||(int)$exP['dibuat_oleh']===$uid)){$curK=explode(',',$exP['kelas_peserta']??'');$allK=[];$q=$conn->query("SELECT DISTINCT kelas FROM students WHERE kelas<>''");while($x=$q->fetch_assoc())$allK[]=$x['kelas'];$q=$conn->query("SELECT nama_kelas FROM kelas");while($x=$q->fetch_assoc())if(!in_array($x['nama_kelas'],$allK))$allK[]=$x['nama_kelas'];usort($allK,function($a,$b){return kelasRank($a)-kelasRank($b);});?>
<div class="card"><h2>🎓 Kelas Peserta — <?=e($exP['mapel'])?> <?=e($exP['kelas'])?></h2>
<form method="post"><input type="hidden" name="action" value="buat_template"><input type="hidden" name="exam_id" value="<?=$pesId?>">
<div style="display:flex;flex-wrap:wrap;gap:10px"><?php foreach($allK as $kk)echo '<label style="font-weight:normal;margin:0"><input type="checkbox" name="kelas_peserta[]" value="'.e($kk).'" '.(in_array($kk,$curK)?'checked':'').'> '.e($kk).'</label>';?></div>
<button class="btn green">📤 Kirim & Buat Template di Google Drive</button> <a class="btn gray" href="?p=setup">Batal</a></form></div>
<?php }} ?>
<div class="card"><h2>Ujian Tersimpan</h2><table><tr><th>Mapel</th><th>Kelas</th><th>Kelas Peserta</th><th>Moda</th><th>Link & Akses</th><th>Aksi</th></tr>
<?php $w=$isAdmin?'':' WHERE dibuat_oleh IN (0,'.$uid.')';$q=$conn->query("SELECT e.*,(SELECT COUNT(*) FROM results r WHERE r.exam_id=e.id) pes FROM exams e".$w." ORDER BY id DESC");while($x=$q->fetch_assoc()){echo '<tr><td>'.e($x['mapel']).'</td><td class="c">'.e($x['kelas']).'</td><td class="c">'.e($x['kelas_peserta']?:'-').'</td><td class="c">'.e($x['mode']?:'-').'</td><td style="font-size:11px">'.($x['link_siswa']?'<a href="'.e($x['link_siswa']).'" target="_blank">🔗 Link Siswa</a> • ':'').($x['link_guru']?'<a href="'.e($x['link_guru']).'" target="_blank">👨‍🏫 Link Guru</a>':'').($x['pin_guru']?'<br>PIN: <b>'.e($x['pin_guru']).'</b>':'').($x['token']?' • Token: <b>'.e($x['token']).'</b>':'').'</td><td><a class="btn" style="margin:0;padding:3px 8px" href="?p=setup&edit='.$x['id'].'">✏</a> <a class="btn green" style="margin:0;padding:3px 8px" href="?p=setup&peserta='.$x['id'].'">🎓</a> <form method="post" style="margin:0;display:inline"><input type="hidden" name="action" value="tarik_hasil"><input type="hidden" name="exam_id" value="'.$x['id'].'"><button class="btn" style="margin:0;padding:3px 8px" title="Tarik hasil dari Google">🔄</button></form> <form method="post" style="margin:0;display:inline"><input type="hidden" name="action" value="del_exam"><input type="hidden" name="exam_id" value="'.$x['id'].'"><button class="btn red" style="margin:0;padding:3px 8px">✖</button></form></td></tr>';}?></table></div>
<script>
...
echo '<tr><td>Persentase Ketercapaian Klasikal</td>';foreach($st['perQ'] as $c)echo '<td class="c">'.($N?round($c/$N*100):0).'%</td>';echo '</tr>';
echo '<tr><td>Nilai Tertinggi</td><td class="c" colspan="'.$examSel['jumlah_soal'].'"><b>'.$maxS.'</b></td></tr>';
echo '<tr><td>Nilai Terendah</td><td class="c" colspan="'.$examSel['jumlah_soal'].'"><b>'.$minS.'</b></td></tr>';
echo '<tr><td>Nilai Rata-rata</td><td class="c" colspan="'.$examSel['jumlah_soal'].'"><b>'.number_format($st['rata'],1).'</b></td></tr>';
echo '<tr><td>Tingkat Kesukaran</td>';foreach($tkArr as $p)echo '<td class="c">'.($p<0.3?'Sukar':($p<=0.7?'Sedang':'Mudah')).'</td>';echo '</tr>';
echo '<tr><td>Daya Pembeda</td>';foreach($dpArr as $dp)echo '<td class="c">'.number_format($dp,2).'</td>';echo '</tr>';
echo '<tr><td>Kriteria Soal</td>';foreach($krArr as $kr)echo '<td class="c">'.$kr.'</td>';echo '</tr>';
echo '</table>';
